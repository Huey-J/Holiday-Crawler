...
def getHolidayData(url, solYear):
    # GET Data
    ...
    # Create SQL File
    f = open(solYear + '_' + url + '.sql', 'w')
    f.write("-- 총 %d개\n" % len(items))
    f.write("INSERT INTO `holiday` (`holiday_type`, `date_name`, `holiday`, `date`, `created_date_time`, `updated_date_time`)\n\tVALUES\n")
...
def getSeasonData(url, solYear):
    kst = "0120"
    sunLongitude = "285"
    # GET Data
    FULL_URL = BASE_URL + url + "?solYear=%s&serviceKey=%s&numOfRows=%s&kst=%s&sunLongitude=%s" % (solYear, SERVICE_KEY, NUM_OF_ROWS, kst, sunLongitude)
    response = requests.get(FULL_URL)
...
def holiday_crawler(solYear):
    getHolidayData(ANNIVERSARY_URL, solYear)
    getHolidayData(NATIONAL_HOLIDAY_URL, solYear)
    getHolidayData(EXCLUDED_SEASON_URL, solYear)
    getSeasonData(SEASON_URL, solYear)


holiday_crawler("2024")
